//
//  NSLayoutConstraint+CBDebugAdditions.m
//  CBonry
//
...
#import "NSLayoutConstraint+CBDebugAdditions.h"
#import "CBConstraint.h"
#import "CBLayoutConstraint.h"

@implementation NSLayoutConstraint (CBDebugAdditions)
...
#if TARGET_OS_IPHONE || TARGET_OS_TV
        descriptionMap = @{
            @(CBLayoutPriorityDefaultHigh)      : @"high",
            @(CBLayoutPriorityDefaultLow)       : @"low",
            @(CBLayoutPriorityDefaultMedium)    : @"medium",
            @(CBLayoutPriorityRequired)         : @"required",
            @(CBLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(CBLayoutPriorityDefaultHigh)                 : @"high",
            @(CBLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(CBLayoutPriorityDefaultMedium)               : @"medium",
            @(CBLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(CBLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(CBLayoutPriorityDefaultLow)                  : @"low",
            @(CBLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(CBLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(CB_key)] && [obj CB_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj CB_key]];
    }
...
- (NSString *)description {
    ...
    if (self.priority != CBLayoutPriorityRequired) {
